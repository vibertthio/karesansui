...
function initScene() {
	...
	gui
		.add(effectController, "mouseSize", 0.2, 10.0, 0.1)
...
function initLayout() {
	...
	const rockZ = lerp(
		...
		BOUNDS_HALF,
		-BOUNDS_HALF,
		circularWavePosition[index].y
	);
	rockPosition = new THREE.Vector3(
		rockX * globalScale,
		rockPositionY * globalScale,
		rockZ * globalScale
	);
}
...
function smoothWater() {
	const currentRenderTarget = gpuCompute.getCurrentRenderTarget(
		heightmapVariable
	);
	const alternateRenderTarget = gpuCompute.getAlternateRenderTarget(
		heightmapVariable
	);
...
function sceneUpdate(deltaTime, elapsedTime) {
	if (
		...
		heightmapVariable.material.uniforms.uTime
	) {
